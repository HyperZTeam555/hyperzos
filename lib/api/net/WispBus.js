...
class WispBus {
    ...
    setUpstreamProvider(upstreamProvider) {
        for (const [globalID, mapping] of this.clientMappings.entries()) {
            if (mapping.handler !== this.upstream)
                continue;
            try {
                mapping.handler.send(wisp.createWispPacket({
    ...
    registerServer(id, regex, handler) {
    ...
    handleIncomingPacket(sourceID, packet) {
    ...
    handleOutgoingPacket(virtualServerID, packet, packetCallBack) {
        const parsed = wisp.parseIncomingPacket(packet);
    ...
    getFakeWispProxySocket() {
        ...
        class FakeSocket extends EventTarget {
            ...
            send(data) {
                ...
                bus.handleOutgoingPacket(this.virtualServerID, pkt, (response) => {
