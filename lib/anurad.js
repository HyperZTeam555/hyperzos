...
class HyperZd extends Process {
    ...
    constructor(pid) {
        super();
        this.pid = pid;
        HyperZdHelpers.setStage("anurad");
    }
    async addInitScript(script) {
        const initScript = new HyperZdInitScript(script, anura.processes.state.procs.length);
    ...
    constructor(script, pid, args = []) {
        ...
        this.window.addEventListener("message", async (event) => {
            if (event.data.type === "init") {
                ...
                this.info.name ||= "HyperZd Script " + this.pid;
                this.info.description ||= "HyperZd script with PID " + this.pid;
                this.info.provides ||= [];
                await this.info.depend();
                await this.info.start();
                HyperZdHelpers.setStage(this.info.name);
            }
        });
