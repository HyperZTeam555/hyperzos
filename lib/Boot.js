...
channel.addEventListener("message", (msg) => {
    ...
    if (msg.data === "blackmanthunderstorm") {
        ...
        document.write(`
            ...
            <body>
            <div id="wrapper">
            <h1>HyperZOS 4 is already running in another tab</h1>
...
window.addEventListener("load", async () => {
        ...
        if (isValidUUID(milestone.split("\n")[0])) {
            ...
            if (!stored) {
                ...
                if (anura.settings.get("use-sw-cache")) {
                    ...
                    tracker.innerText = "HyperZ is updating your system...";
                    try {
            ...
            console.log("HyperZ update poisoning detected...");
        }
    }
    const isGitRev = (hash) => /^[0-9a-f]{7,40}$/i.test(hash);
    if (commit) {
    ...
    anura.registerLib(new HyperZdHelpersLib());
    // Register anurad, claiming PID 1
    const anurad = new HyperZd(1);
    anura.anurad = anurad;
    anura.processes.register(anurad);
    HyperZdHelpers.setReady("anura.anurad");
    Object.entries(anura)
        .filter(([_, v]) => v !== undefined)
        .map(([k]) => "anura." + k)
        .forEach(HyperZdHelpers.setReady);
    /**
     ...
     * introduced in HyperZOS 4 2.0.0. This is to ensure that
     * users who have been using HyperZOS 4 for a while can
    ...
    if (directories) {
        ...
        if (needsMigration.length > 0) {
            anura.notifications.add({
                title: "HyperZ Update",
                description: "HyperZOS 4 has been updated to a new version. Users are recommended to change the installation directory of their apps and libraries to /usr/ to ensure consistency with new installations.",
                timeout: "never",
                buttons: [
                    {
                        text: "Migrate Now",
    ...
    /**
     * These directories are required for HyperZ to function
    ...
    anura.registerLib(new HyperZGlobalsLib());
...
document.addEventListener("anura-boot-completed", async () => {
    HyperZdHelpers.setStage("anura.boot");
    if (anura.settings.get("oobe-complete")) {
...
document.addEventListener("anura-login-completed", async () => {
    ...
    HyperZdHelpers.setReady("anura.dialog");
    ...
    // Copy apps from config.json to the filesystem directory structure first
    for (const appPath of anura.config.apps) {
        try {
            const appName = appPath.split('/').pop();
            const destPath = `${directories["apps"]}/${appName}`;
            try {
                await anura.fs.promises.stat(destPath);
            } catch (e) {
                // Directory doesn't exist, copy it
                const appDir = appName.replace('.app', '');
                const files = await fetch(`/__api__/fs/read?path=${encodeURIComponent(appPath)}`).then(r => r.json());
                if (files && files.files) {
                    await anura.fs.promises.mkdirp(destPath);
                    for (const file of files.files) {
                        const content = await fetch(`${appPath}/${file}`).then(r => r.blob());
                        await anura.fs.promises.writeFile(`${destPath}/${file}`, content);
                    }
                }
            }
        } catch (e) {
            console.error('Failed to copy app:', appPath, e);
        }
    }
    ...
    await launcher.init();
    await taskbar.init();
    // Register external apps from config AFTER launcher is initialized so they appear in the launcher
    for (const app of anura.config.apps) {
        await anura.registerExternalApp(`${directories["apps"]}/${app.split('/').pop()}/`);
    }
    // Mark init as complete BEFORE user customizations so taskbar and alttab update properly
    anura.initComplete = true;
    HyperZdHelpers.setReady("anura.initComplete");
    ...
    HyperZdHelpers.setReady("anura.systray");
    anura.ui.theme.apply();
...
async function bootx86() {
    ...
    HyperZdHelpers.setReady("anura.x86");
    anura.settings
        .get("user-xapps")
        .forEach((stub) => {
    ...
    HyperZdHelpers.setStage("anura.bootx86");
}
async function bootUserCustomizations() {
    ...
    if ((await fetch("/fs/")).status === 404) {
        ...
        anura.notifications.add({
            title: "HyperZ Error",
            description: "HyperZ has detected a system fault and booted in safe mode. Click this notification to enter the recovery app.",
            timeout: "never",
            ...
            if (files) {
                for (const file of files) {
                    ...
                    try {
                        ...
                        anura.logger.error("HyperZ failed to load a script " + e);
                    }
                }
            }
        }
    ...
    try {
        ...
        for (const file of files) {
            try {
                ...
                anura.logger.error("HyperZ failed to load a lib", e);
            }
        }
    }
    catch (e) {
            ...
            for (const file of files) {
                ...
                if (type === "DIRECTORY") {
                    try {
                        ...
                        anura.logger.error("HyperZ failed to load an app", e);
                    }
                }
                else {
    ...
    HyperZdHelpers.setStage("anura.bootUserCustomizations");
}
