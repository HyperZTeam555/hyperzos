...
class HyperZ {
    ...
    static async new(config) {
        ...
        const fs = new HyperZFilesystem([fsProvider]);
        ...
        const anuraPartial = new HyperZ(fs, settings, config, hdd, net);
        return anuraPartial;
    }
    apps = {};
    libs = {};
    ...
    async registerExternalApp(source) {
        ...
        if (manifest.type === "auto" || manifest.type === "manual") {
            const normalizedSource = new URL(source, location.href).href;
            const app = new ExternalApp(manifest, normalizedSource);
            await anura.registerApp(app); // This will let us capture error messages
            return app;
        }
...
//# sourceMappingURL=HyperZ.js.map