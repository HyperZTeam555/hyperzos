...
export class ProviderLoader {
	 ...
	 * @param {HyperZ} anura - The HyperZ instance
	...
	async build(app, config = {}, provider = "anureg") {
		...
		for (let i = 0; i < using.length; i++) {
			switch (using[i]) {
				...
				case "basepath":
					args.push(
						this.anura.settings.get("directories")["opt"] + "/" + app.package,
					);
					break;
				default:
...
export function buildLoader(anura, basepath) {
	if (!basepath) {
		basepath =
			anura.settings.get("directories")["opt"] + "/anura.persistence/providers";
	}
