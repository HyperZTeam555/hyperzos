	...
	if (directories) {
		...
		if (needsMigration.length > 0) {
			anura.notifications.add({
				title: "HyperZ Update",
				description:
					"HyperZOS 4 has been updated to a new version. Users are recommended to change the installation directory of their apps and libraries to /usr/ to ensure consistency with new installations.",
				timeout: "never",
				buttons: [
					{
						text: "Migrate Now",
...
async function bootUserCustomizations() {
	...
	if ((await fetch("/fs/")).status === 404) {
		...
		anura.notifications.add({
			title: "HyperZ Error",
			description:
				"HyperZ has detected a system fault and booted in safe mode. Click this notification to enter the recovery app.",
			timeout: "never",
